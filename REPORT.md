...
| ID | 질문 | 실패 유형 | 원인 |
|----|------|----------|------|
| 2 | BI 스탠다드 포함 항목 | 도구 과호출 (search_service 불필요) | get_service_detail만 기대했으나 search_service도 함께 호출 |
| 6 | PRJ-2024-003 환불 가능? | 필수 사실 누락 ("취소가 불가") | 긴급 작업 취소 불가를 "환불 불가"로만 표현, "취소가 불가" 표현 미사용 |
| 9 | 로고 베이직 원본 소스? | 도구 미호출 + 필수 누락 | 근거 문서로만 답변하여 "별도 구매" 표현 누락 |
| 10 | PRJ-2024-001 저작권? | 필수 누락 ("완료") | "이전되었습니다"로 답변했으나 "완료" 단어 미사용 |
| 11 | 긴급 로고 추가 비용 | 라우팅 오류 (REVISION) | "긴급"을 수정 관련으로 오분류, SERVICE_INFO가 정답 |
| 12 | AI vs 사람 작업? | 도구 미호출 | OTHER로 라우팅되어 에스컬레이션 되었으나 escalate_to_agent 도구 미호출 |

### macro F1 및 혼동행렬 (v2 기준)

**라우팅 분류 리포트** (`sklearn.metrics.classification_report`):

| 카테고리 | precision | recall | F1-score | support |
|---------|-----------|--------|----------|---------|
| DELIVERY_COPYRIGHT | 1.00 | 1.00 | 1.00 | 4 |
| OTHER | 1.00 | 1.00 | 1.00 | 1 |
| PAYMENT_REFUND | 1.00 | 1.00 | 1.00 | 4 |
| REVISION | 1.00 | 0.75 | 0.86 | 4 |
| SERVICE_INFO | 0.80 | 1.00 | 0.89 | 4 |
| **macro avg** | **0.96** | **0.95** | **0.94** | **15** (2건 오류 제외) |

**혼동행렬:**

| 실제 ↓ \ 예측 → | DELIVERY | OTHER | PAYMENT | REVISION | SERVICE |
|----------------|----------|-------|---------|----------|---------|
| DELIVERY_COPYRIGHT | **4** | 0 | 0 | 0 | 0 |
| OTHER | 0 | **1** | 0 | 0 | 0 |
| PAYMENT_REFUND | 0 | 0 | **4** | 0 | 0 |
| REVISION | 0 | 0 | 0 | **3** | 1 |
| SERVICE_INFO | 0 | 0 | 0 | 0 | **4** |

- 오분류 1건: ID 11 "긴급 로고 추가 비용" — 기대 SERVICE_INFO → 실제 REVISION으로 분류 (긴급 작업을 수정 관련으로 오인)
